...
if isequal(method,'ced')
    ...
    if prm.planewise
        for i = 1 : dim(3)
            imhere = im(:,:,i);
            im(:,:,i) = cohenhdiff(imhere,prm.ced.dt,prm.ced.maxniter,prm.ced.kappa);       
        end;
    else
        % true 3D
        im = cohenhdiff(im,prm.ced.dt,prm.ced.maxniter,prm.ced.kappa,'opt','num');       
...
elseif isequal(method,'dirced')
    ...
    if prm.planewise
        for i = 1 : dim(3)
            imhere = im(:,:,i);
            im(:,:,i) = dircohenh(imhere,prm.dirced.diameter,prm.h,prm.gpu);
        end;
    else
        % true 3D
         im = dircohenh(im,prm.dirced.diameter,prm.h);
    end;
